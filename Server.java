...
import team3.src.util.ConfigData; // for reading and writing the config file.
//SSL 
...
public class Server extends AbstractServer {
	 ...
	 */
	private static ServerClientProtocol protocol = ServerClientProtocol
			.getProtocol("");
	...
	private static void sendWait(Socket client, AbstractResponse msg)
			throws IOException {
	...
	public static void main(String[] args) {
		// save start time
		Date startTime = new Date(); // auto set to *now*
		...
		try {
			...
			ConfigData cd = ConfigData.getConfigData();
			// this needs to be an object that clients can see too.

			cd.generateServerInformationSet();

			sslserversocket = (SSLServerSocket) sslserversocketfactory
					.createServerSocket(41152);
			...
			while (isRunning()) {
				try {
					...
					/*
					{
						...
						String str[] = client.getSupportedCipherSuites();
						int len = str.length;
						...
						client.setEnabledCipherSuites(set);

						str = client.getEnabledCipherSuites();
					...
					// If this is a terminating message, then prevent further
					// client requests from hitting the server
					if (message.getMsgType() == Message.Type.ATOMIC
							&& message.read().equals("Terminate")) {
						finished();
						closeConnection();
					}
					/*
					 * Check to see if we need to have this client wait on
					 * message If No clients are waiting, do not send wait
					 * message If clients are waiting and this is a read/write
					 * operation If running protocol on this message would
					 * result in failure Send failure message (DO NOT QUEUE)
					 * Otherwise send wait message and place in queue If this is
					 * an OTHER type message, then we do not send wait message
					 * but put it in the queue
					 */
					if (priorityPool.size() > 0
							&& (newSocket.getMode() == Message.Type.READ
									|| newSocket.getMode() == Message.Type.WRITE
									|| newSocket.getMode() == Message.Type.DELETE || newSocket
									.getMode() == Message.Type.ATOMIC
									&& newSocket.getPriority() > 0)) {
						AbstractResponse sendMsg = protocol
								.generateWaitMessage();
						// logger.log(sendMsg);
						try {
			...
			socketConsumer.join();
								 ...
								 * logger.log(errorHandler.handleSocketIOException
								 * (SocketError.CONNECTION_INIT_ERROR));
								 */
		} catch (InterruptedException e) { /*
											 * logger.log(
											 * "Unable to join on threads... Alert Admin about this."
											 * );
											 */
		}
	...
	/**
	 * Handles the listening for clients to the system, parses the input for
	 * priority
	 * 
	 * @author Joir-dan
	 * 
	...
	protected static class PriorityServerThread extends Thread {
		...
		public static final PriorityServerThread getThread() {
			return (singleton != null) ? singleton
					: (singleton = new PriorityServerThread());
		}
			...
			while (true) {
				while (canRun() || priorityPool.size() > 0) {
					...
					if (first != null) {
						...
						/*
						 * In case you are wondering why I make this if
						 * statement call even though I have a switch-case
						 * statement involving the SocketMode enum, consider
						 * this: If we are working with a "Req Hello", or a
						 * "Req Bye", we don't want to go through the process of
						 * running through embedded synchronization blocks...
						 * too much overhead and locking resources that the
						 * worker thread may need. Because of this, I decided to
						 * just throw an AssertionError on the variable
						 * first.mode if OTHER or null are found as first.mode's
						 * value.
						 */
						if (first.getMode() != Message.Type.ATOMIC) {
							synchronized (readerIn) {
								synchronized (writerIn) {
									if (readerIn || (!readerIn && !writerIn)) {
										// TODO: Deal with locks and
										// priorities...
										switch (first.getMode()) {
										case READ: {
											if (getReadLock()) {
												// README!! WORKER MUST RELEASE
												// LOCK IN WORKER THREAD!!
							...
							// Its a socket of type SocketMode.OTHER... so, no
							// worries about modifying data...
							// TODO: Just create workerThread... and let it
							// go...
		...
		/**
		 * Method called to gain access to read lock for reading data from this
		 * server's directory (read directory, or read files)
		 * 
		...
		/**
		 * Method called to gain access to write lock for editing this server's
		 * directory (either adding or deleting data)
		 * 
		...
		public boolean getWriteLock() {
			synchronized (writerIn) {
				synchronized (readerIn) {
					// logger.log("WRITE LOCK OBTAINED? "+ !(readerIn ||
					// readSemaphore > 0 || writerIn));
		...
		/**
		 * Method that is called upon completing a read to decrement reader
		 * semaphore. If the worker calling this thread is the last reader, it
		 * is responsible for reader cleanup (releasing read lock for writer to
		 * come in).
		 */
		...
		/**
		 * Method that is called upon completing a write to release lock and
		 * allow other clients to have access to data
		 */
		...
		private class WorkerThread extends AbstractServerThread {
			...
			private WorkerThread(PrioritySocket pSocket) {
				super(pSocket.getID());
				this.socket = pSocket;
				this.protocol = ServerClientProtocol.getProtocol(pSocket
						.getID());
			}

			/**
			 * This is the main communication method of the WorkerThread. It
			 * will grab a message from the thread's client's instream, log the
			 * message, process message, log outgoing message, send message to
			 */
			public void work() {
				...
				while (weCantStop()) {
					...
					try {
						...
						synchronized (isAlive) {
							isAlive = (isAlive) ? !protocol.isTerminated()
									: isAlive;
						}
					} catch (IOException e) {
						out.println("IO");
			...
			public void run() {
				...
				try {
					setStreams(socket.getClient());
				} catch (IOException e) { /*
										 * TODO: Use that
										 * AbstractErrorHandler... catch this
										 * error
										 */
				}
				...
				case ATOMIC:
					...
					break;
				default:
					throw new AssertionError(socket.getMode()); // WE SHOULDNT
																// GET HERE...
																// FAIL FAST...
				}
			}
