...
public class AbstractClient {
	...
	/**
	 * Checks to see whether our client process is done (terminate has been
	 * called)
	 * 
	 * @return true if our process is not done, false otherwise
	 */
	protected static boolean checkIsNotDone() {
		return isNotDone;
	}
	 ...
	 */
	protected static void thenClientIsDone() {
		isNotDone = false;
	}
	...
	protected static void initData() {
		try {
			...
			throw new AssertionError("WHERE IS THE CONFIG.INI FILE??");
		}
	}
	 ...
	 */
	private static String nextReqNum() {
		return String.format("%03d", ++requestNo % 1000);
	}
	 ...
	 */
	protected static final String generateID() {
		return String.format("%03d", (int) (1000 * Math.random()));
	}

	/**
	 * Tests whether we are on our first request. On the first time this is
	 * called, it sets the variable to true but returns false.
	 * 
	...
	protected static final String[] parseCommand(String[] args)
			throws IllegalCommandException {
		if (args.length == 0)
			return parseCommand();
		if (args[0]
				.matches("file (put|get)|directory list|hello|terminate|delete|my directory"))
			return args;
		else
	 ...
	 * @throws IllegalCommandException
	 *             if it is an ill-formed command
	 */
	protected static final String[] parseCommand()
			throws IllegalCommandException {
		...
		try {
			...
			if ((command = commandScanner
					.findInLine("file (put|get)|directory list|hello|terminate|my directory|delete")) != null)
				cmdList.add(command);
			else {
				throw new IllegalCommandException();
			}
			try {
				while ((command = commandScanner.useDelimiter(" ").next()) != null)
					cmdList.add(command);
	...
	protected static void getDirectory(String[] args)
			throws IllegalCommandException {
		...
		try {
			...
			for (int i = start; i < nmax; i = i + 2) {
				out.format(
						format,
						file.list()[i],
						((i + 1 < file.list().length) && (i + 1 < nmax)) ? file
								.list()[i + 1] : " ");
			}
	...
	/**
	 * Abstraction of a client thread. This class will do the communication
	 * between the AbstractClient and the Abstract Server.
	 * 
	...
	protected static abstract class AbstractClientThread extends Thread {
		/**
		 * Boolean flag that indicates whether this clientThread is still
		 * executing.
		 */
		 ...
		 * 
		 * @param requestID
		 *            the identifier for our thread
		 * @param data
		 *            extracted from config.ini wrapper ConfigData
		 * @param message
		 *            the command we are passing to the server
		 */
		protected AbstractClientThread(String requestID, ConfigData data,
		 ...
		 *            the command we are passing to the server
		 * @param filename
		 *            the name of the file we are either getting or receiving
		 */
		protected AbstractClientThread(String requestID, ConfigData data,
				String[] message, String filename) throws UnknownHostException,
				IOException {
			super(requestID);
			...
			// TODO:
			// logger.log("Logger initialized for ClientThread: "+requestID);
		}
		 ...
		 * @throws IOException
		 *             if unable to read from in-stream buffer
		 */
		protected AbstractResponse grabFromServer() throws IOException,
				JAXBException {
		 ...
		 */
		protected void writeToServer(AbstractMessage message)
		 ...
		 *             This means our config.ini file is corrupted
		 */
		 ...
		 * @return chunk size
		 * @throws NumberFormatException
		 *             if our config.ini file is corrupted
		 */
		 ...
		 *             If we are unable to find server
		 * @throws IOException
		 *             if we cannot gain access to serverSocket I/O streams
		 */
		protected void initConnection() throws UnknownHostException,
				IOException {
			sslsocket = (SSLSocket) sslsocketfactory.createSocket(getHost(),
					getPort());

			// set algorithm connection priorities
			// /*
			{
				...
				String str[] = sslsocket.getSupportedCipherSuites();
				...
				sslsocket.setEnabledCipherSuites(set);

				str = sslsocket.getEnabledCipherSuites();
